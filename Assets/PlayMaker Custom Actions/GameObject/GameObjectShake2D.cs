...
namespace HutongGames.PlayMaker.Actions
{
	...
	[Tooltip("Shake any object but without Z axis - good for 2D")]
	[HelpUrl("http://hutonggames.com/playmakerforum/index.php?topic=9928")]
	public class GameObjectShake2D : FsmStateAction
	{
		...
		public override void OnEnter ()
		{
			...
			originalPosFirstPass = objTransform.localPosition;
			...
			if (loop.Value == true)
		{
			...
			if (shakeTime <= 0f)
			{
				...
				objTransform.localPosition = originalPosFirstPass;
		...
		void DoObjShake()
		{
			...
			if (shakeTime > 0f || loop.Value == true){
				...
				if (lerpOn == true) {
					...
					tempV3 = originalPos + Random.insideUnitCircle * shakeAmount.Value;
					tempV3.z = originalPosFirstPass.z;
					objTransform.localPosition = tempV3;
		...
		void DoObjShakeLerp()
		{
			...
			if (Vector2.Distance(newPos,objTransform.localPosition)<=shakeAmount.Value/30) {
				
				newPos = originalPos + Random.insideUnitCircle * shakeAmount.Value;
			}
			
			if (animationCurvebool == true){
				
				tempV3 = Vector2.Lerp(objTransform.localPosition, newPos , lerpCurve.curve.Evaluate(shakeTime));
				tempV3.z = originalPosFirstPass.z;
				objTransform.localPosition = tempV3;
				...
				tempV3 = Vector2.Lerp(objTransform.localPosition, newPos , t);
				tempV3.z = originalPosFirstPass.z;
				objTransform.localPosition = tempV3;
			}
