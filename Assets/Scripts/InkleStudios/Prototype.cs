...
// Slightly modified version with the addition of OneFrameReturnDelay bool variable allowing one frame delay during the returnToPool call
...
namespace Com.InkleStudios
{
	...
	/// instantiate a copy in the sam
	/// e place in the hierarchy.
	...
	public class Prototype : MonoBehaviour
	{
		...
		/// <summary>
		/// The return to the pool is delayed one frame for the instance being returned to have time to catch onReturnToPool. Set this to true only if necessary.
		/// </summary>
		[HideInInspector]
		public bool OneFrameReturnDelay;
		...
		public void ReturnToPool ()
		{
			...
			if (OneFrameReturnDelay) {
				Debug.Log ("delayed return");
				if (this.OnReturnToPool != null)
					this.OnReturnToPool ();
				
				StartCoroutine ("cr_DelayedReturn");
			} else {
		...
		IEnumerator cr_DelayedReturn()
		{
			yield return new WaitForEndOfFrame ();

			_originalPrototype.AddToPool (this,false);
		}
		...
		void AddToPool (Prototype instancePrototype,bool withCallback=true)
		{
			if (!isOriginalPrototype)
